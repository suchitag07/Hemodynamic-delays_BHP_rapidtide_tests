        ...
        if self.peakfittype == "fastquad" or self.peakfittype == "COM":
            ...
            if not (
                ...
                if maxlag_init <= (self.lagmin - rangeextension - binwidth):
                    ...
                    print("BEFORE REFINEMENT (INITLAGHIGH):","lagmin=", self.lagmin,"lagmax=", self.lagmax,"maxlag_init=", maxlag_init) #--added by SG 4/22
        ...
        if self.peakfittype != "None":
            ...
            if (self.lagmin > maxlag) or (maxlag > self.lagmax):
                ...
                if self.lagmin > maxlag:
                    ...
                    maxlag = self.lagmin #- maxlag is being clipped to lower bound when failed - OK SG 4/22
                else:
                    failreason |= self.FML_FITLAGHIGH
                    print("BAD LAG AFTER REFINEMENT (FITLAGHIGH):","lagmin=", self.lagmin,"lagmax=", self.lagmax,"(refined)maxlag=", maxlag) #-- added by SG 4/22
                    maxlag = self.lagmax #- maxlag is being clipped to upper bound when failed - OK SG 4/22
                fitfail = True
